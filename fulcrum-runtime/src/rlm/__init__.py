"""Tiny rlm-compatible kernel shim for Fulcrum."""
...
async def host_request(request_type: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    """Send a typed request to the Fulcrum host and await its reply.
...
async def run(prompt: str, **kwargs: Any) -> RLMSpawnHandle:
    """Spawn a recursive Fulcrum child and return once its task is admitted.
