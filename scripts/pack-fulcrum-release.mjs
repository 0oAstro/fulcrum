...
// TODO: Remove this R2 tarball packer once fulcrum and its internal workspace
...
const defaultBaseUrl = process.env.FULCRUM_DOWNLOAD_BASE_URL;
const publicPackageName = process.env.FULCRUM_PACKAGE_NAME || "fulcrum";
const publicCommandName = process.env.FULCRUM_CMD || "fulcrum";
const releaseChannels = new Set(["stable", "beta"]);

const releasePackages = [
	{ packageDir: "ai", publicName: undefined, artifactName: "fulcrum-ai" },
	{ packageDir: "tui", publicName: undefined, artifactName: "fulcrum-tui" },
	{ packageDir: "agent", publicName: undefined, artifactName: "fulcrum-core" },
...
function parseArgs(args) {
	const parsed = {
		...
		version: undefined,
		tarballUrlTemplate: process.env.FULCRUM_TARBALL_URL_TEMPLATE,
	};
	...
	for (let i = 0; i < args.length; i += 1) {
		...
		switch (arg) {
			...
			case "--tarball-url-template": {
				const value = args[i + 1];
				if (!value) throw new Error("--tarball-url-template requires a value");
				parsed.tarballUrlTemplate = value;
				i += 1;
				break;
			}
			case "--help":
	...
	if (!parsed.baseUrl) {
		throw new Error("--base-url or FULCRUM_DOWNLOAD_BASE_URL is required");
	}
...
function printHelp() {
	console.log(`Usage: node scripts/pack-fulcrum-release.mjs --base-url url [--channel stable|beta] [--version x.y.z] [--tarball-url-template template] [--out-dir path]

Creates private npm tarballs for R2 distribution:

  <out-dir>/artifacts/fulcrum-<version>.tgz
  <out-dir>/artifacts/fulcrum-ai-<version>.tgz
  <out-dir>/artifacts/fulcrum-core-<version>.tgz
  <out-dir>/artifacts/fulcrum-tui-<version>.tgz
...
function releaseTarballUrl(baseUrl, version, tarballFile, template) {
	if (template) {
		return template
			.replaceAll("{base}", baseUrl)
			.replaceAll("{version}", version)
			.replaceAll("{tag}", `v${version}`)
			.replaceAll("{file}", tarballFile);
	}
...
function main() {
	...
	const releaseVersion = args.version || normalizeVersion(process.env.FULCRUM_VERSION || cliPackage.version);
	...
	for (const releasePackage of releasePackages) {
		...
		internalPackageUrls.set(
			sourcePackageName,
			releaseTarballUrl(args.baseUrl, releaseVersion, artifactFile, args.tarballUrlTemplate),
		);
	}
	...
	writeJson(join(artifactsDir, manifestName), {
		...
		tarball: releaseTarballUrl(
			args.baseUrl,
			releaseVersion,
			artifactFiles.get("coding-agent"),
			args.tarballUrlTemplate,
		),
