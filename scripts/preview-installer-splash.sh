...
frames="${1:-${FULCRUM_SPLASH_PREVIEW_FRAMES:-300}}"
case "$frames" in
	""|*[!0-9]*)
...
preview_dir=$(mktemp -d "${TMPDIR:-/tmp}/fulcrum-splash-preview.XXXXXX")
...
i=0
while [ "$i" -lt "$1" ]; do
	fulcrum_screen "Installing Fulcrum" "Downloading Fulcrum$(fulcrum_pulse)" "Fetching the verified package." ""
	sleep 0.18
	i=$((i + 1))
done
EOF
