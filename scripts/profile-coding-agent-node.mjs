...
const agentDirEnvName = "FULCRUM_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "FULCRUM_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific FULCRUM_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force FULCRUM_OFFLINE=1 / FULCRUM_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.FULCRUM_OFFLINE = "1";
		env.FULCRUM_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "fulcrum-startup-benchmark-")) : undefined;
