#!/usr/bin/env bash
#
# Build Fulcrum binaries for all platforms locally.
...
#     fulcrum-darwin-arm64.tar.gz
#     fulcrum-darwin-x64.tar.gz
#     fulcrum-linux-x64.tar.gz
#     fulcrum-linux-arm64.tar.gz
#     fulcrum-windows-x64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == "windows-x64" ]]; then
        bun build --compile --external koffi --target=bun-$platform ./dist/bun/cli.js --outfile binaries/$platform/fulcrum.exe
    else
        bun build --compile --external koffi --target=bun-$platform ./dist/bun/cli.js --outfile binaries/$platform/fulcrum
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == "windows-x64" ]]; then
        # Windows (zip)
        echo "Creating fulcrum-$platform.zip..."
        (cd $platform && zip -r ../fulcrum-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating fulcrum-$platform.tar.gz..."
        mv $platform fulcrum && tar -czf fulcrum-$platform.tar.gz fulcrum && mv fulcrum $platform
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf $platform
    if [[ "$platform" == "windows-x64" ]]; then
        mkdir -p $platform && (cd $platform && unzip -q ../fulcrum-$platform.zip)
    else
        tar -xzf fulcrum-$platform.tar.gz && mv fulcrum $platform
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    echo "  binaries/$platform/fulcrum"
done
