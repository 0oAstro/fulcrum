...
fulcrum_test_cols=80
fulcrum_test_rows=24

fulcrum_read_terminal_size() {
	fulcrum_screen_cols="$fulcrum_test_cols"
	fulcrum_screen_rows="$fulcrum_test_rows"
}
...
print_render_meta() {
	...
	printf '__META__ %s cols=%s rows=%s layout_show_logo=%s lab_width=%s render_lab_width=%s compact=%s visible=%s content_height=%s\\n' \\
		"$label" "$fulcrum_screen_cols" "$fulcrum_screen_rows" "$fulcrum_screen_layout_show_logo" \\
		"$fulcrum_screen_layout_lab_width" "$fulcrum_screen_render_lab_width" "$fulcrum_screen_compact" "$visible" "$content_height"
}

render_case() {
	fulcrum_screen_title="Installing Fulcrum"
	fulcrum_screen_detail="Fetching the verified package."
	fulcrum_screen_question=
	fulcrum_screen_frame=1
	fulcrum_screen_cols="$1"
	fulcrum_screen_rows="$2"
	...
	fulcrum_screen_frame=2
	fulcrum_screen_cols="$3"
	fulcrum_screen_rows="$4"
	fulcrum_refresh_screen_layout_mode
...
screen_case() {
	...
	fulcrum_screen "Installing Fulcrum" "Installing Fulcrum" "Fetching the verified package." ""
...
Finalizing npm install."
	for progress_frame in 1 24 25 48 49 200; do
		fulcrum_animation_frame="$progress_frame"
		printf '__PROGRESS__ %s\t%s\t%s\\n' "$progress_frame" "$(fulcrum_animation_status "Installing Fulcrum" "$progress_details" static)" "$(fulcrum_animation_detail "$progress_details")"
	done
}
...
const tempDir = mkdtempSync(join(tmpdir(), "fulcrum-installer-render-"));
...
function assertInstallerProgress(progress) {
	...
	for (const [index, expectedDetail] of expectedDetails.entries()) {
		...
		check(
			progress[index].status === "Installing Fulcrum...",
