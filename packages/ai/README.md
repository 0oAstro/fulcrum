<h1 align="center">Fulcrum AI</h1>

<p align="center">
  LLM provider toolkit.
</p>

Release docs use the Fulcrum package name. The source workspace manifest still keeps an inherited package name until the namespace migration is complete.
...
```bash
npm install fulcrum-ai
```

TypeBox exports are re-exported from `fulcrum-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from 'fulcrum-ai';
...
```typescript
import { Type, Tool, StringEnum } from 'fulcrum-ai';
...
```typescript
import { stream, validateToolCall, Tool } from 'fulcrum-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from 'fulcrum-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from 'fulcrum-ai';
...
```typescript
import { getModel, stream } from 'fulcrum-ai';
...
```typescript
import { getProviders, getModels, getModel } from 'fulcrum-ai';
...
```typescript
import { Model, stream } from 'fulcrum-ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are Fulcrum thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from 'fulcrum-ai';
...
```typescript
import { getModel, complete, Context } from 'fulcrum-ai';
...
```typescript
import { Context, getModel, complete } from 'fulcrum-ai';
...
- OAuth login flows are not supported in browser environments. Use the `fulcrum-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from 'fulcrum-ai';
...
```bash
npx fulcrum-ai login              # interactive provider selection
npx fulcrum-ai login anthropic    # login to specific provider
npx fulcrum-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `fulcrum-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from 'fulcrum-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from 'fulcrum-ai/oauth';
...
```typescript
import { getModel, complete } from 'fulcrum-ai';
import { getOAuthApiKey } from 'fulcrum-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `fulcrum-ai`
