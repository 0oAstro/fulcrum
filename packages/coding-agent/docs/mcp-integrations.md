...
Connect external services (Linear, Notion, …) to Fulcrum over the
[Model Context Protocol](https://modelcontextprotocol.io).

Consistent with Fulcrum's single-tool design, MCP integrations are **not**
...
Credentials are stored once in `~/.fulcrum/auth.json` under `mcp:<name>`.
...
Add it under `mcpServers` in `~/.fulcrum/settings.json` (or project
...
`~/.fulcrum/skills/acme/`) with the standard Python-skill layout:
...
`pyproject.toml` (depends on `mcp`, `httpx`, and `fulcrum-runtime`):

```toml
[project]
name = "fulcrum-skill-acme"
version = "0.1.0"
requires-python = ">=3.10"
dependencies = ["mcp", "httpx", "fulcrum-runtime"]
...
- **Custom kernel + name collisions.** The kernel import name is the `server`
  value. On a custom `FULCRUM_KERNEL_PYTHON` that already has an unrelated PyPI
