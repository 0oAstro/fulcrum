> Fulcrum can create TUI components. Ask it to build one for your use case.
...
Fulcrum shows compact image metadata and does not emit terminal graphics for images.
...
Customize the inline working indicator shown while Fulcrum is streaming a response.
...
// Restore Fulcrum's default spinner
ctx.ui.setWorkingIndicator();
```
