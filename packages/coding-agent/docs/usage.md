...
On Windows Terminal, Alt+Enter is fullscreen by default. Remap it as described in [Terminal setup](terminal-setup.md) if you want Fulcrum to receive the shortcut.

Configure delivery in [Settings](settings.md) with `steeringMode` and `followUpMode`.

## Sessions

Sessions are saved automatically as flat JSONL files under `~/.fulcrum/sessions/`. Each session header records its working directory, which the session picker uses for project-scoped views.

```bash
fulcrum -c                  # Continue most recent session
fulcrum -r [path|id]        # Browse sessions or resume one directly
fulcrum --no-session        # Ephemeral mode; do not save
fulcrum --fork <path|id>    # Fork a session into a new session file
```
...
Normal interactive sessions are persistent agents backed by isolated worker processes. Closing the TUI detaches the client; use `fulcrum agents`, `fulcrum list`, or `fulcrum attach <agent>` to find and reattach to running work. `fulcrum stop <agent>` stops one root agent, while `fulcrum shutdown` stops all workers and the local supervisor.
...
- `.fulcrum/SYSTEM.md` for a project
- `~/.fulcrum/SYSTEM.md` globally
...
fulcrum agents
fulcrum list [--all]
fulcrum attach <agent>
fulcrum stop <agent>
fulcrum rename <agent> <name>
fulcrum send <agent> <message>
fulcrum schedule <list|add|cancel>
fulcrum status
fulcrum doctor [--fix]
fulcrum shutdown [--force]

fulcrum package install <source> [--local]
fulcrum package remove <source> [--local]
fulcrum package list
fulcrum package update [source]
fulcrum update [--force]
fulcrum config
```

See [Fulcrum Packages](packages.md) for package sources and security notes.
...
Use `fulcrum session export <file> [output]` to export a session to HTML.
...
```bash
fulcrum --no-extensions -e ./my-extension.ts
```
...
|----------|-------------|
| `FULCRUM_CODING_AGENT_DIR` | Override config directory; default is `~/.fulcrum` |
...
| `PI_SKIP_VERSION_CHECK` | Skip the Fulcrum version update check at startup. This prevents the release manifest request |
| `FULCRUM_DOWNLOAD_BASE_URL` | Override the Fulcrum release manifest and tarball base URL |
| `PI_CACHE_RETENTION` | Set to `long` for extended prompt cache where supported |
| `FULCRUM_TRACES_BASE_URL` | Override the Fulcrum trace upload API base URL |
| `FULCRUM_KERNEL_PYTHON` | Use an existing Python environment with `ipykernel` instead of bootstrapping `~/.fulcrum/kernel-venv` |
| `VISUAL`, `EDITOR` | External editor for Ctrl+G |

The remaining `PI_*` variables are compatibility names still read by the current runtime. They do not change the application name, command, or default `~/.fulcrum` configuration path.

## Design Principles

Fulcrum keeps the model-facing tool surface small while making the IPython runtime powerful and composable. The built-in `ipython` tool provides durable state, project command execution, Python skills, MCP-backed integrations, and the native `rlm` delegation API without presenting each capability as a separate model tool.
...
Extensions, skills, prompt templates, themes, and Fulcrum packages remain the primary customization surfaces. They can add project-specific workflows, custom tools and UI, permission policies, provider integrations, and orchestration patterns around the built-in runtime.

Fulcrum preserves MIT attribution to pi-mono for its upstream lineage, but upstream Pi product claims and limitations do not describe the current Fulcrum architecture.
