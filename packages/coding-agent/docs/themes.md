> Fulcrum can create themes. Ask it to build one for your setup.
...
Fulcrum loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.fulcrum/themes/*.json`
- Project: `.fulcrum/themes/*.json`
...
On first run, Fulcrum detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.fulcrum/themes
vim ~/.fulcrum/themes/my-theme.json
```
...
**Hot reload:** When you edit the currently active custom theme file, Fulcrum reloads it automatically for immediate visual feedback.
...
Fulcrum uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Fulcrum falls back to the nearest approximation.
