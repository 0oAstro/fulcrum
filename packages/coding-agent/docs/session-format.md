...
```
~/.fulcrum/sessions/<session-id>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.fulcrum/sessions/`.

Fulcrum also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, Fulcrum uses the `trash` CLI to avoid permanent deletion.
...
### Base Message Types (from Fulcrum AI)
...
### Extended Message Types (from Fulcrum Coding Agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if generated by Fulcrum (legacy field name)
