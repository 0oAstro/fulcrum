...
function socketPathForRun(): string {
	...
	if (process.platform === "win32") {
		return `\\\\.\\pipe\\fulcrum-multiclient-bench-${process.pid}-${socketCounter}`;
	}
	return `/tmp/fulcrum-multiclient-${process.pid}-${socketCounter}.sock`;
}
...
const streamCorpus = repeatedText(STREAM_CHARS, "Fulcrum daemon multi-client streaming benchmark. ");
...
function createAttachResponseForMessages(
	...
	const state = {
		...
		sessionDir: "/tmp/fulcrum-benchmark/sessions",
		...
		for (let offset = 0, sequence = 1; offset < streamCorpus.length; sequence++) {
			...
			const compact = createCompactAssistantDelta({
				...
				meta: {
					...
					protocol: { name: "fulcrum.daemon", version: 2 },
					activeSessionId: "active-benchmark",
					sequence,
...
async function runAttachV2(
	...
	const cacheRoot = await mkdtemp(join(tmpdir(), "fulcrum-attach-v2-bench-"));
...
async function loadSessionFixture(sessionFile: string): Promise<{
	...
	const tempDir = await mkdtemp(join(tmpdir(), "fulcrum-session-bench-"));
...
function printResults(
	...
	console.log("Fulcrum daemon multi-client benchmark");
	console.log(details);
	console.table(
		results.map((result) => ({
...
function printSessionLoad(sessionFile: string, result: SessionLoadResult): void {
	console.log("Fulcrum real-session benchmark");
...
async function generateSessionFixture(sizeMiB: number): Promise<GeneratedSessionFixture> {
	const directory = await mkdtemp(join(tmpdir(), `fulcrum-generated-${sizeMiB}mib-`));
