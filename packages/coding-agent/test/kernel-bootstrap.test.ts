...
describe("kernel bootstrap", () => {
	beforeEach(async () => {
		...
		tempDir = mkdtempSync(join(tmpdir(), "fulcrum-kernel-bootstrap-"));
		process.env.HOME = tempDir;
		process.env.PATH = originalEnv.PATH ?? "";
		delete process.env.FULCRUM_KERNEL_PYTHON;
		delete process.env.FULCRUM_KERNEL_VENV;
	...
	it("returns the configured kernel venv directory", () => {
		...
		process.env.FULCRUM_KERNEL_VENV = venv;

		expect(getKernelVenvDir()).toBe(venv);
	});

	it("bootstraps a missing venv with uv, ipykernel, fulcrum-runtime, and default extra packages", async () => {
		...
		expect(log).toContain("fulcrum-runtime");
	...
	it("installs sibling Python skill dependencies when package and directory names differ", async () => {
		...
		writeFileSync(
			...
			`[project]
name = "fulcrum-skill-attach-image"
version = "0.1.0"
`,
		);
		const dependentSkill = createPythonSkillWithDependency("orchestration-heartbeat", "fulcrum-skill-attach-image");
		process.env.FULCRUM_KERNEL_VENV = venv;
...
`,
	...
	it("uses FULCRUM_KERNEL_PYTHON as an override contract", async () => {
		...
		process.env.FULCRUM_KERNEL_PYTHON = overridePython;

		await expect(ensureKernelPython()).resolves.toBe(overridePython);
	});

	it("allows FULCRUM_KERNEL_PYTHON missing Python skill imports", async () => {
		const overridePython = join(tempDir, "override-python");
		const pythonSkill = createPythonSkill();
		writeFakePython(overridePython, ["ipykernel", "rlm", ...DEFAULT_RLM_EXTRA_IMPORT_NAMES]);
		process.env.FULCRUM_KERNEL_PYTHON = overridePython;
	...
	it("rejects FULCRUM_KERNEL_PYTHON missing default extra packages", async () => {
		const overridePython = join(tempDir, "override-python");
	...
	it("rejects FULCRUM_KERNEL_PYTHON with a stale rlm runtime", async () => {
		const overridePython = join(tempDir, "override-python");
		writeFakePython(overridePython, ["ipykernel"]);
		process.env.FULCRUM_KERNEL_PYTHON = overridePython;

		await expect(ensureKernelPython()).rejects.toThrow(/current fulcrum-runtime with callable rlm\.run/);
	});

	it("rejects FULCRUM_KERNEL_PYTHON with a legacy harness API", async () => {
		const overridePython = join(tempDir, "override-python");
		writeExecutable(
			overridePython,
			[
	...
	it("fails an invalid FULCRUM_KERNEL_PYTHON without bootstrapping", async () => {
		const overridePython = join(tempDir, "override-python");
		writeFakePython(overridePython, []);
		process.env.FULCRUM_KERNEL_PYTHON = overridePython;
