...
const OWNED_WORKER_ENV = "FULCRUM_INTERNAL_OWNED_WORKER";
const OWNED_RECOVERY_DESCRIPTOR_ENV = "FULCRUM_INTERNAL_OWNED_RECOVERY_DESCRIPTOR";
const OWNED_PROFILE_ENV = "FULCRUM_INTERNAL_OWNED_PROFILE";
...
function isStartupBenchmark(environment: NodeJS.ProcessEnv): boolean {
	const value = environment.FULCRUM_STARTUP_BENCHMARK?.toLowerCase();
...
export async function runOwnedSessionWorkerFrontend(
	...
	const recoveryDescriptorPath = join(tmpdir(), `fulcrum-owned-${process.pid}-${randomUUID().slice(0, 12)}.json`);
	...
	const anonymousRpcIdPrefix = `fulcrum-owned-${randomUUID()}`;
...
export async function maybeRunOwnedSessionWorkerFrontend(
	...
	if (!forceLegacyFrontend && process.env.FULCRUM_INTERNAL_LEGACY_OWNED_WORKER_FRONTEND !== "1") {
		return false;
	}
