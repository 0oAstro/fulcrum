...
export function assertNodeVersion(io: NodeVersionGuardIO): boolean {
	...
	io.log(`fulcrum requires Node ${MIN_NODE_VERSION} or newer, but the active Node is v${io.version}.`);
	io.log("");
	io.log(`  1. Install Node ${MIN_NODE_VERSION}+ (e.g. "nvm install 22 && nvm use 22", or from https://nodejs.org)`);
	io.log("  2. Reinstall fulcrum under that Node so the command resolves to it:");
	io.log("     Use the same package manager or installer that supplied this Fulcrum installation.");
	io.exit(1);
	return false;
}
