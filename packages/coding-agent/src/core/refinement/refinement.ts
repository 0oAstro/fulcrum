...
export const REFINEMENT_CUSTOM_TYPE = "fulcrum.refinement";
...
const REFINEMENT_SYSTEM_PROMPT = `You are Fulcrum's /refine continual harness subsystem.
...
skills, and subagent specs that lets Fulcrum improve reusable behavior
...
- The default editable continual harness store is local to the current Fulcrum session. Use it for session-specific progress, active task state, current-run coordination notes, temporary blockers, and project facts that should not affect other sessions.
...
const AUTO_REFINE_REVIEW_SYSTEM_PROMPT = `You are Fulcrum's automatic /refine review gate.
...
export function formatHarnessStateForPrompt(
	...
	const lines = [
		...
		"",
		"Local continual harness entries belong to this Fulcrum session. Global continual harness entries persist across Fulcrum sessions.",
