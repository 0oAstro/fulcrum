...
function getBuiltinThemes(): Record<string, ThemeJson> {
	if (!BUILTIN_THEMES) {
		...
		const fulcrumPath = path.join(themesDir, "fulcrum.json");
		const darkPath = path.join(themesDir, "dark.json");
		const lightPath = path.join(themesDir, "light.json");
		BUILTIN_THEMES = {
			fulcrum: JSON.parse(fs.readFileSync(fulcrumPath, "utf-8")) as ThemeJson,
...
function getDefaultTheme(): string {
	// Fulcrum is dark-first; only fall back to light when the terminal is light.
	return detectTerminalBackground() === "light" ? "light" : "fulcrum";
}
