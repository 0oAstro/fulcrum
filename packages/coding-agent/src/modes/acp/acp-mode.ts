...
/**
 * Split ACP prompt blocks into the text and images fulcrum accepts.
 *
...
export async function runAcpModeWithConnection(
	...
	const handle = acp
		...
		.onRequest("initialize", async () => ({
			...
			agentInfo: { name: "fulcrum", title: "Fulcrum", version: VERSION },
			// Advertise fulcrum extras under a namespaced key: ACP reserves
		...
		.onRequest("session/new", async (ctx: any) => {
			...
			if (session) {
				throw new Error(
					"fulcrum ACP mode hosts one session per connection; " +
						"start another fulcrum process for a second session",
				);
			}
			// fulcrum's cwd is fixed at startup by the session it was launched
			...
			// Subscribe for the session lifetime, not per prompt turn: fulcrum
			...
			const unsubscribe = connection.subscribe((event) => {
				...
				if (event.type === "heartbeats_changed") {
					notify({ sessionUpdate: "session_info_update", _meta: fulcrumMeta({ heartbeatsChanged: true }) });
					return;
				}
			...
			return {
				sessionId,
				...(cwdMismatch ? { _meta: fulcrumMeta({ cwd: cwdMismatch }) } : {}),
			};
		})
		.onRequest("session/prompt", async (ctx: any) => {
			...
			try {
				...
				if (failure && !abort.signal.aborted) {
					throw new Error(`fulcrum turn failed: ${failure}`);
				}
