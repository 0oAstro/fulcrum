...
export const FULCRUM_BUILD_ID_ENV = "FULCRUM_BUILD_ID";
export const FULCRUM_LAUNCHER_PATH_ENV = "FULCRUM_LAUNCHER_PATH";
...
export function getDaemonRuntimeIdentity(environment: NodeJS.ProcessEnv = process.env): DaemonRuntimeIdentity {
	...
	const launcher = environment[FULCRUM_LAUNCHER_PATH_ENV];
	return {
		buildId: environment[FULCRUM_BUILD_ID_ENV] ?? bundledBuildId() ?? `release-${VERSION}`,
