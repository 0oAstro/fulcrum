 ...
 * - ~/.fulcrum/presets.json (global)
 * - <cwd>/.fulcrum/presets.json (project-local)
 *
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".fulcrum", "presets.json");
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify("No presets defined. Add presets to ~/.fulcrum/presets.json", "warning");
			return;
		}
